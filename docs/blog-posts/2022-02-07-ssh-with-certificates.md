# SSH with Certificates on Windows
...
Next, we use [`ssh-keygen`][ssh-keygen] to **sign** the client's public key (`client_key.pub`), with the private key of the keypair we generated earlier (`user_ca`). (Note: if you have added the private key to `ssh-agent`, see [below](#optional-adding-the-user_ca-key-to-ssh-agent).
...
- [`-n`][-n]: Comma-separated list of usernames which the client is allowed to log into the host as. Usernames must already exist on the host. Also known as 'principals'. If this option is omitted, the client will not be able to login.
- [`-V`][-V]: Validity interval of the certificate. If omitted, the certificate will be valid forever.

Finally, we copy the files `client_key` (private key), `client_key.pub` (public key) and `client_key-cert.pub` (signed public key) to the client. Usually, these files are placed in `%UserProfile%\.ssh`.

## 3. Login

Now, test the SSH login from the client!

`ssh you@your.domain.name`
...
You may now safely delete the private key and the signed public key.

To check the keys which have been added to the agent, run `ssh-add -l`.

From now on, `ssh-agent` will automatically supply the signed public key and private key in commands such as `ssh`.
...
Finally, on the client, when we connect, we use the following command:
